      ...
      <div>
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 20px;">
          ${data.media_plan_allocation.map(p => {
            ...
            return `<div style="background: ${bg}; border-left: 4px solid ${border}; padding: 20px; border-radius: 12px;"><p style="color: ${text}; font-weight: 600; margin: 0 0 8px;">${p.channel}</p><p style="margin: 0; color: #ccc;">Index: <strong>${p.index}</strong></p><p style="margin: 0; color: #ccc;">Budget Allocation: <strong>£${budget}</strong></p></div>`;
          }).join('')}
        </div>
      </div>
    </div>
